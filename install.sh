...
if [ -f /etc/arch-release ]; then
  ...
  sudo pacman -S git curl unzip zsh tmux neovim python python-neovim luarocks
fi

# install dependencies if ubuntu
if [ -f /etc/lsb-release ]; then
  echo "Installing dependencies for Ubuntu..."
  sudo apt install git curl unzip zsh tmux neovim python python-neovim luarocks
fi

# install dependencies if mac
if [ "$(uname)" == "Darwin" ]; then
  echo "Installing dependencies for Mac..."
  brew install git curl unzip zsh tmux neovim python python-neovim luarocks
fi
...
echo "Install Meslo Nerd Font from ~/.dotfiles/fonts/MesloLGS_NF_Regular.ttf"
